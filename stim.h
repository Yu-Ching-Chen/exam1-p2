...
SC_MODULE(stim) {
  ...
  sc_in_clk i_clk;
  sc_out<bool> o_rst;
  sc_fifo_out<sc_uint<sc_ufixed_fast<5, 0> > o_x;
  sc_fifo_in<sc_ufixed_fast<8, 3>> i_y;
  ...
  void stim_gen() {
    ...
    for (int n = 0; n < 64; n++) {
        o_x.write(x_input_signal[n*M-1]);
        wait();
        o_x.write(x_input_signal[n*M]);
        wait();
        o_x.write(x_input_signal[n*M+1]);
        wait();
    }
